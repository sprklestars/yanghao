...
class StrategyConfig:
    ...
    )  # Keywords that trigger extraction phase
...
class StrategyEngine:
    ...
    def select_strategy(
        self, category: TargetCategory, context: Optional[dict] = None
    ) -> StrategyConfig:
        ...
        sorted_strategies = sorted(available_strategies, key=lambda s: s.priority, reverse=True)
            ...
            if account_health == "yellow":
                ...
                for strategy in sorted_strategies:
                    if strategy.strategy_type == StrategyType.FRIEND_ADDING:
                        strategy.friend_requests_per_day = max(
                            1, strategy.friend_requests_per_day - 2
                        )
            elif account_health == "red":
                # Only use passive strategies
                sorted_strategies = [
                    s for s in sorted_strategies if s.strategy_type == StrategyType.GROUP_MIXING
                ]
    ...
    def update_conversation_state(
        self, conversation_id: str, incoming_message: str, current_stage: str
    ) -> ConversationState:
    ...
    def get_next_stage(self, conversation_id: str) -> str:
        ...
        current_idx = (
            stage_order.index(state.current_stage) if state.current_stage in stage_order else 0
        )
