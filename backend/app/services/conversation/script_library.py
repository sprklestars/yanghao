...
class ScriptLibrary:
    ...
    def get_templates(
        self, category: str, stage: str, language: str, limit: int = 5
    ) -> list[ScriptTemplate]:
        ...
        sorted_templates = sorted(available, key=lambda t: t.effectiveness_score, reverse=True)
            ...
            for template in templates_list:
                if template.id == template_id:
                    ...
                    template.effectiveness_score = (
                        1 - alpha
                    ) * template.effectiveness_score + alpha * reward
                    break
    ...
    "probing": {
        "private_investigator": [
            {
                "vi": (
                    ...
                    "Mình cần tìm hiểu một số chuyện."
                ),
                "zh": "你知道有谁做私人侦探服务吗?我需要调查一些事情。",
                "en": (
                    "Do you know anyone who does private investigation? "
        ...
        "currency_exchanger": [
            ...
            },
            {
                ...
                "en": (
                    "Do you know a reliable place to exchange money? I need to change about $2000."
                ),
            },
        ],
        "freelancer": [
...
async def humanize_with_llm(
    original_text: str, persona_config: dict, context: Optional[str] = None
) -> str:
...
Persona: {persona_config.get("name", "User")}, {persona_config.get("age", 28)} years old,
...
def select_best_template(
    library: ScriptLibrary, category: str, stage: str, language: str, use_ab_testing: bool = True
